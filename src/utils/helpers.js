...
export function shortenList(list, number = 3) {
  ...
  if (length === 1) return list[0]
    ...
    return `${JSON.stringify(...list.slice(0, length - 2))} and ${
      list[length - 1]
    }`
  else
    return `${JSON.stringify(...list.slice(0, number - 1))} and ${
      length - number
    } others`
}
...
export async function getPubKeyTagWithRelay(pubkey) {
  ...
  if (event && event.seen_on && event.seen_on.length) {
    let random =
      event.seen_on[Math.floor(Math.random() * event.seen_on.length)]
    base.push(random)
  }
  return base
}
