...
zoo`.split("\n");
  ...
  var require_utils = __commonJS({
    "node_modules/@scure/bip39/node_modules/@noble/hashes/utils.js"(exports) {
      ...
      var hexes4 = Array.from({ length: 256 }, (v, i) =>
      ...
      function wrapConstructor3(hashConstructor) {
        const hashC = (message) =>
          hashConstructor().update(toBytes3(message)).digest();
      ...
      function wrapConstructorWithOpts(hashCons) {
        const hashC = (msg, opts) =>
          hashCons(opts).update(toBytes3(msg)).digest();
        const tmp = hashCons({});
        hashC.outputLen = tmp.outputLen;
        hashC.blockLen = tmp.blockLen;
  ...
  var require_hmac = __commonJS({
    "node_modules/@scure/bip39/node_modules/@noble/hashes/hmac.js"(exports) {
      ...
      var hmac3 = (hash2, key, message) =>
        new HMAC3(hash2, key).update(message).digest();
      exports.hmac = hmac3;
      ...
      var SHA2563 = class extends _sha2_js_1.SHA2 {
        ...
        process(view, offset) {
          ...
          for (let i = 0; i < 64; i++) {
            ...
            const T1 =
              (H + sigma1 + Chi3(E, F, G) + SHA256_K3[i] + SHA256_W3[i]) | 0;
            const sigma0 =
  ...
  var Point = class {
    ...
    static fromHex(hex2) {
      ...
      if (len === fieldLen) return this.fromCompressedHex(bytes2);
  ...
  function hexToBytes(hex2) {
    ...
    if (hex2.length % 2)
      throw new Error(
        "hexToBytes: received invalid unpadded hex" + hex2.length
      );
    const array = new Uint8Array(hex2.length / 2);
    for (let i = 0; i < array.length; i++) {
      const j = i * 2;
  ...
  function ensureBytes(hex2) {
    return hex2 instanceof Uint8Array
      ? Uint8Array.from(hex2)
      : hexToBytes(hex2);
  }
  ...
  var SchnorrSignature = class {
    ...
    static fromHex(hex2) {
      ...
      if (bytes2.length !== 64)
        throw new TypeError(
          `SchnorrSignature.fromHex: expected 64 bytes, not ${bytes2.length}`
        );
  ...
  var utils = {
    ...
    sha256: async (...messages) => {
      if (crypto2.web) {
        const buffer = await crypto2.web.subtle.digest(
          "SHA-256",
  ...
  utils.hmacSha256Sync = (key, ...msgs) =>
    hmac(sha2562, key, utils.concatBytes(...msgs));
    ...
    get privateExtendedKey() {
      ...
      return base58check2.encode(
        this.serialize(
          ...
          concatBytes2(new Uint8Array([0]), priv)
        )
      );
    }
    ...
    constructor(hash2, _key) {
      ...
      pad.set(
        key.length > this.iHash.blockLen
          ? hash2.create().update(key).digest()
          : key
      );
      for (let i = 0; i < pad.length; i++) pad[i] ^= 54;
      this.iHash.update(pad);
  ...
  async function privateKeyFromX(username, caip10, sig, password) {
    ...
    let inputKey = await sha256(
      utils.hexToBytes(sig.toLowerCase().startsWith("0x") ? sig.slice(2) : sig)
    );
    let info = `${caip10}:${username}`;
    let salt = await sha256(
  ...
  // index.ts
  utils.hmacSha256Sync = (key, ...msgs) =>
    hmac2(sha256, key, utils.concatBytes(...msgs));
