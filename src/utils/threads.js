...
function searchAndUpdateThreads(threads, route, ...events) {
  ...
  if (events[events.length - 1].replies?.length)
    events[0].latest_created_at = events[events.length - 1].replies.reduce(
      (acc, curr) => Math.max(acc, curr[0].latest_created_at),
      events[events.length - 1].latest_created_at
    )
  else
    events[0].latest_created_at = events[events.length - 1].latest_created_at
  let event = events[0]
  ...
  for (let i = 0; i < threads.length; i++) {
    ...
    if (refs.length === 0) {
      ...
      if (
        thread0Refs.length &&
        thread0Refs[thread0Refs.length - 1] === event.id
      )
      ...
      if (pushThreads.length === 0) {
        ...
        let threadIndex = thread.findIndex(
          (threadEvent) => refs[refs.length - 1] === threadEvent.id
        )
        if (threadIndex >= 0) {
          ...
          threadLast.created_at < event.created_at &&
          threadLast.replies?.length
        ) {
  ...
  if (
    unshiftThreads.length === 0 &&
    pushThreads.length === 0 &&
    insertThreads.length === 0 &&
    replyInsertThreads.length === 0
  )
    return false
  if (
    pushThreads.length > 1 ||
    insertThreads.length > 1 ||
    (pushThreads.length && insertThreads.length)
  ) {
    return false
  }
  if (unshiftThreads.length) {
    if (unshiftThreads.length === 1 && events.length === 1) {
      ...
      if (thread[thread.length - 1].replies?.length)
        thread[0].latest_created_at = thread[thread.length - 1].replies.reduce(
          (acc, curr) => Math.max(acc, curr[0].latest_created_at),
          thread[thread.length - 1].latest_created_at
        )
      else
        thread[0].latest_created_at =
          thread[thread.length - 1].latest_created_at
      ...
      for (let j = unshiftThreads.length - 1; j >= 0; j--) {
        ...
        insertThreads = insertThreads.map(({ thread, threadIndex }) =>
          thread > j
            ? { thread: thread - 1, threadIndex }
            : { thread, threadIndex }
        )
        replyInsertThreads = replyInsertThreads.map((i) => (i > j ? i - 1 : i))
      }
      event.latest_created_at = event.replies.reduce(
        (acc, curr) => Math.max(acc, curr[0].latest_created_at),
        event.latest_created_at
      )
      events = [event]
      if (
        pushThreads.length === 0 &&
        ...
        addSorted(
          threads,
          events,
          (a, b) => a[0].latest_created_at < b[0].latest_created_at
        )
        return true
      }
    }
  ...
  if (pushThreads.length) {
    ...
    if (thread[thread.length - 1].replies?.length) {
      if (unshiftThreads.length === 1 && events.length === 1) {
        ...
        insertThreads = insertThreads.map(({ thread, threadIndex }) =>
          thread > unshiftThreads[0]
            ? { thread: thread - 1, threadIndex }
            : { thread, threadIndex }
        )
        replyInsertThreads = replyInsertThreads.map((i) =>
          i > unshiftThreads[0] ? i - 1 : i
        )
  ...
  if (insertThreads.length) {
    ...
    try {
      slicedReply[0].latest_created_at =
        slicedReply[slicedReply.length - 1].latest_created_at
    } catch (e) {
      let idx = threads.findIndex(
        (thread) =>
          thread[0].id ===
  ...
  if (replyInsertThreads.length) {
    for (let i in replyInsertThreads) {
      ...
      if (unshiftThreads.length === 1 && events.length === 1) {
        let inserted = searchAndUpdateThreads(
          thread[thread.length - 1].replies,
          route,
          ...threads[unshiftThreads[0]]
        )
        if (inserted) {
          threads.splice(unshiftThreads[0], 1)
          thread[0].latest_created_at = thread[
            thread.length - 1
          ].replies.reduce(
            (acc, curr) => Math.max(acc, curr[0].latest_created_at),
            thread[thread.length - 1].latest_created_at
...
export function addToThread(threads, event, route = '', reorder) {
  ...
  if (searchAndUpdateThreads(threads, route, event)) {
    if (reorder)
      threads.sort(
        (a, b) =>
          Number(b[0].latest_created_at) - Number(a[0].latest_created_at)
      )
  } else
    addSorted(
      threads,
