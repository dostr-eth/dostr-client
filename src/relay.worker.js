...
function advanceTicketsToDb() {
  let tickets = JSON.parse(
    JSON.stringify(
      Object.values(dbTicketQueue).filter(
        (ticket) => ticket.relayWork.eventsToSave.length
      )
    )
  )
  Object.values(dbTicketQueue)
    .filter((ticket) => ['stream', 'update'].includes(ticket.type))
    .forEach((ticket) => (ticket.relayWork.eventsToSave = []))
...
function cancelTicket(ticket) {
  ...
  for (let url in ticket.relayWork.relays)
    if (relays[url]) relays[url].processTicket(ticket)
}
...
async function relayHandler(url) {
  ...
  function startSub(subName) {
    ...
    if (subControl[subName].mode === 'stream')
      startStreamTickets(subName, true)
    else if (subControl[subName].mode === 'call')
      startNextTicket(subName, true)
  }
  ...
  function startNextTicket(subName, subStarted = false) {
    ...
    if (!ticket) closeSub(subName)
      ...
      let filter = getSubFilter(subName)
      // let { filter } = ticket.relayWork
      if (!subStarted && filter !== subControl[subName].filter)
        subControl[subName].sub.sub(filter)
      subControl[subName].filter = filter
      // subControl[subName].mode = 'query'
      subControl[subName].timeout = setTimeout(
        () => handleEose(subName, true),
        5000
      )
    }
  }
  ...
  function handleEose(subName, manual = false) {
    ...
    if (mode === 'call') {
      ...
      if (!manual && ticket.relayWork.eose)
        self.postMessage({
          ticket: { id: ticket.id, eose: ticket.eose, success: true },
        })
    } else if (mode === 'stream') {
      let tickets = queue
      tickets.forEach((ticket) => {
  ...
  async function addTicketToQueue(ticket, subName) {
    ...
    else if (status === 'connected' && !subControl[subName].sub)
      startSub(subName)
    else if (status === 'connected' && subControl[subName].mode === 'stream')
      startStreamTickets(subName)
  }

  function updateTicket(ticket, subName) {
    // remove ticket from queue
    // console.log('updateTicket', ticket, subName, JSON.parse(JSON.stringify(subControl)))
    let index = subControl[subName].queue.findIndex(
      (tck) => tck.id === ticket.id
    )
    // console.log('updateTicket', index, ticket, subName, JSON.parse(JSON.stringify(subControl)))
    if (index !== -1) {
      if (
        JSON.stringify(subControl[subName].queue[index].relayWork.filter) ===
        JSON.stringify(ticket.relayWork.filter)
      )
        return
      subControl[subName].queue[index].relayWork.filter =
        ticket.relayWork.filter
      if (!subControl[subName].sub) startSub(subName)
      else if (subControl[subName].mode === 'stream')
        startStreamTickets(subName)
    }
    // console.log('updateTicket', ticket, subName, JSON.parse(JSON.stringify(subControl)))
  }
  ...
  function cancelTicket(ticket, subName) {
    if (
      subControl[subName].ticket &&
      subControl[subName].ticket.id === ticket.id
    ) {
  ...
  function publishTicket(ticket) {
    ...
    pub.on('ok', () => {
      ...
      checkTicketForCompletion(ticket)
      self.postMessage({
        ticket: { id: ticket.id, results: true, success: true },
      })
    })
    pub.on('seen', () => {
    ...
    pub.on('failed', (reason) => {
      console.log(
        `${url} PUBLISH FAILED: event ${event.id} failed because ${reason}`
      )
      self.postMessage({
        ticket: { id: ticket.id, results: false, success: true },
      })
    })
  }
