<template>
  ...
  >
    ...
    >
      ...
      <q-icon
        v-if="showFollowing && isFollow"
        name="visibility"
        :size="headerMode ? 'sm' : 'xs'"
        color="secondary"
      >
        <q-tooltip> following </q-tooltip>
      </q-icon>
    </div>
    <div
      class="flex row no-wrap items-center"
      :style="headerMode ? 'gap: .3rem;' : 'gap: .1rem;'"
    >
      <BaseButtonNIP05
        :pubkey="pubkey"
        :button-size="headerMode ? 'sm' : 'xs'"
      />
      <div
        v-if="$store.getters.NIP05Id(pubkey)"
        style="opacity: 0.9; font-size: 90%; font-weight: 300; line-height: 90%"
      >
        {{ headerMode ? niceNIP05 : niceShortNIP05 }}
      </div>
    </div>
    <div
      v-if="headerMode && lnurlToLnAddr($store.getters.profileLud06(pubkey))"
      style="opacity: 0.9; font-size: 90%; font-weight: 300; line-height: 90%"
    >
...
export default {
  ...
  computed: {
    niceShortNIP05() {
      let niceNIP05 = this.$store.getters
        .NIP05Id(this.pubkey)
        .split('@')
        .map((el, index) =>
          index === 0 && el === this.$store.getters.name(this.pubkey) ? '' : el
        )
        .join('@')
      return niceNIP05.startsWith('_@')
        ? niceNIP05.replace('_@', '')
        : niceNIP05
    },
    niceNIP05() {
