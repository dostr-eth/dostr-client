  ...
  <div class="lightning-card">
    ...
    >
      <div
        ...
        :style="rowOrColumn === 'row' ? 'gap: .5rem;' : 'gap: 1rem;'"
        class="flex column justify-between"
        :align="rowOrColumn === 'row' ? 'start' : 'center'"
      >
        ...
        >
          <div v-if="bolt11.description">
            <strong style="color: skyblue">summary:</strong>
            {{ bolt11.description }}
          </div>
          <div>
          ...
          <div v-if="bolt11.created">
            <strong style="color: skyblue">creation date:</strong>
            {{ dateUTC(bolt11.created) }}
          </div>
          <div v-if="bolt11.created">
            <strong style="color: skyblue">creation time:</strong>
            {{ timeUTC(bolt11.created) }}
          </div>
          <div v-if="bolt11.expires">
            <strong style="color: orange">expiry date:</strong>
            {{ dateUTC(bolt11.expires) }}
          </div>
          <div v-if="bolt11.expires">
            <strong style="color: orange">expiry time:</strong>
            {{ timeUTC(bolt11.expires) }}
          </div>
        ...
        <div
          class="flex row no-wrap q-pt-xs"
          style="gap: 0.5rem"
          :class="rowOrColumn === 'row' ? ' justify-start' : ' justify-center'"
        >
          <BaseButtonCopy
            :button-text="lnString"
            button-label="copy"
            outline
            class="q-pr-xs"
            @click.stop="updateMode('copy')"
          />
          <q-btn
            label="qr"
            icon="qr_code_2"
      ...
      >
        ...
        <q-tab-panel
          name="wallet"
          class="no-padding flex items-center justify-center full-width"
        >
