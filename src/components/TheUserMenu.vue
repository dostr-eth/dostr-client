  ...
  computed: {
    filteredUserMenuItems() {
      if (!this.$store.state.keys.pub)
        return this.userMenuItems.filter((item) => item.title === 'feed')
      if (!this.compactMode && !this.showCompactModeItems)
        return this.userMenuItems.filter((item) => !item.compactMenuOnly)
