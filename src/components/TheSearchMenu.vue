    ...
    <q-card-section class="q-pa-sm">
      <q-form @submit="searchProfile">
        ...
        >
          <template #append>
            <BaseButtonClear
              :button-text="searchingProfile"
              button-class="text-secondary"
              @clear="searchingProfile = ''"
            />
            <q-btn
      ...
      <div v-if="domainMode">
        ...
        <div v-if="domainDefaultPubkey">
          <h2 class="text-caption text-bold q-my-none">
            {{ $t("nip05Maintainer") }}
          </h2>
  ...
  computed: {
    validSearch() {
      ...
      if (
        ...
        this.bech32ToHex(this.searchingProfile).match(/^[a-f0-9A-F]{64}$/)
      )
        return true
      if (
    ...
    domainUsers() {
      let users = Object.keys(this.domainNames)
        .filter((name) => name !== '_')
        .map((name) => {
          return { name: name, pubkey: this.domainNames[name] }
        })
      return users
    },
    domain() {
  ...
  methods: {
    async searchProfile() {
      if (!this.validSearch) {
        Notify.create({
          message:
            '⚠️ Invalid format! Please enter full Public Key or NIP-05 identifier',
          color: 'negative',
          classes: 'notify',
        })
        return
      ...
      if (
        ...
        else if (this.searchingProfile.startsWith('note'))
        ...
        if (
          ...
          if (this.domainUsers.length || this.domainDefaultPubkey) {
            ...
            if (this.domainUsers.length)
              this.domainUsers.forEach((user) => this.useProfile(user.pubkey))
            this.searching = false
      ...
      Notify.create({
        message:
          '⚠️ No user found! Please enter full Public Key or NIP-05 Identifier and double-check search string',
        color: 'negative',
        classes: 'notify',
      })
    },
    ...
    reorderFollows() {
      this.reorderedFollows = this.$store.state.follows.map((pubkey) => {
        return { pubkey }
      })
    ...
    saveReorder() {
      this.$store.commit(
        'reorderFollows',
        this.reorderedFollows.map((follow) => follow.pubkey)
      )
