    ...
    <div class="flex column no-wrap col full-width">
      <q-item-section ref="postContent">
        <div class="q-py-sm q-px-lg">
          ...
          />
          <BaseRelayRecommend
            v-else-if="event.kind === 2"
            :url="sanitize(event.content)"
          />
          <BaseUserCard
            v-else-if="event.kind === 0"
            :pubkey="event.pubkey"
            :header-mode="true"
          />
      ...
      <div
        v-if="
          ...
          replyDepth !== -1 &&
          event.replies &&
          event.replies.length < 3
        "
        class="full-width"
        style="padding-left: 12px"
      >
        <div
          v-for="thread in event.replies"
          :key="thread[0].id"
...
export default defineComponent({
  ...
  computed: {
    ...
    taggedPubkeys() {
      let pubkeyTags = this.event.tags
    ...
    replyContentHeight() {
      // only including if statement to trigger recalc on resize
      if (this.replyMode && this.resizing && this.$refs.replyContent?.$el)
        return this.$refs.replyContent?.$el?.clientHeight
    ...
    childReplyConnectorStyle() {
      if (this.childReplyHeights?.length) {
        let height =
          this.headerHeight +
          this.postHeight +
          this.childReplyHeights.slice(0, -1).reduce((c, p) => c + p, 0)
      ...
      nextTick(() => {
        setTimeout(() => {
          ...
          if (this.hasReplyChildren) {
            this.childReplyHeights = this.$refs.childReplyContent?.map(
              (div) => div.clientHeight
            )
          }
          this.$emit('resized')
        }, time)
...
.reposts {
  ...
  box-shadow: 0 0 0.3rem -0rem var(--q-accent);
  min-width: 150px;
}
</style>
