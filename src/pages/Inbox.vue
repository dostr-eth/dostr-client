    ...
    <q-list class="q-py-sm q-pr-sm q-gutter-sm">
      ...
      <q-item
        ...
        @click.capture.stop="
          $router.push({
            name: 'messages',
            params: { pubkey: hexToBech32(chat.peer, 'npub') },
          })
        "
      >
...
export default {
  ...
  methods: {
    async start() {
      ...
      this.sub.streamMainIncomingMessages = await streamMainIncomingMessages(
        { authors: [this.$store.state.keys.pub], relays, limit: 1000 },
        null,
        () => {
          this.getChats()
          this.loading = false
      ...
      this.sub.streamMainOutgoingMessages = await streamMainOutgoingMessages(
        { authors: [this.$store.state.keys.pub], relays, limit: 1000 },
        null,
        () => {
          this.getChats()
    ...
    stop() {
      if (this.sub.streamMainIncomingMessages)
        this.sub.streamMainIncomingMessages.cancel()
      if (this.sub.streamMainOutgoingMessages)
    ...
    async getChats() {
      ...
      if (this.allChatsNeverRead)
        this.chats.forEach(({ peer }) =>
          this.$store.commit('haveReadMessage', peer)
        )
    },
