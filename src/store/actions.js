...
export async function restartMainSubscription(store) {
  // get relays
  let relays = Object.keys(store.state.relays).length
  ...
  if (!mainSub.streamMainProfilesFollows)
    mainSub.streamMainProfilesFollows = await streamMainProfilesFollows(
      { authors, relays },
      async (events) => {
        ...
        for (let event of events) {
          if (event.kind === 0) {
            ...
            event.pubkey === store.state.keys.pub &&
            event.kind === 3
          ) {
            ...
            LIMIT 1
          `)
            if (result.length && event.created_at < result[0].created_at)
              return
...
export async function sendChatMessage(store, { now, pubkey, text, tags }) {
  ...
  try {
    ...
    Notify.create({
      message: `⚠️ Could not encrypted message: ${error}`,
      color: 'negative',
      classes: 'notify',
    })
    return
...
export async function publishEvent(store, { unpublishedEvent }) {
  ...
  try {
    ...
    if (!publishResult)
      console.log(`no publish confirmation for ${eventTypeWordy} event`)
    await store.dispatch('addEvent', { event })
    if (event.kind === 0)
      store.dispatch('handleAddingProfileEventToCache', event)
    ...
    Notify.create({
      message: `⚠️ Could not publish ${eventTypeWordy} event: ${error}`,
      color: 'negative',
      classes: 'notify',
    })
    return
      ...
      if (events)
        for (let event of events) {
          if (event.kind === 1 || event.kind === 2)
            helpersMixin.default.methods.interpolateEventMentions(event, store)
          else if (event.kind === 4) {
            event.text = await helpersMixin.default.methods.getPlaintext(
              event,
              store
            )
            helpersMixin.default.methods.interpolateMessageMentions(
              event,
              store
            )
          }
...
export async function handleAddingProfileEventToCache(store, event) {
  if (
    store.state.profilesCache[event.pubkey] &&
    event.created_at <= store.state.profilesCache[event.pubkey].created_at
  )
    return
