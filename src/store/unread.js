...
export default function (store) {
  ...
  const streamMentionsAndMessages = async () => {
    ...
    if (store.state.keys.pub) {
      if (sub.streamMainMentions)
        sub.streamMainMentions.update({
          authors: [store.state.keys.pub],
          relays,
        })
      else
        sub.streamMainMentions = await streamMainMentions(
          { authors: [store.state.keys.pub], relays },
          () => {
      ...
      if (sub.streamMainIncomingMessages)
        sub.streamMainIncomingMessages.update({
          authors: [store.state.keys.pub],
          relays,
        })
      else
        sub.streamMainIncomingMessages = await streamMainIncomingMessages(
          { authors: [store.state.keys.pub], relays },
          (events) => {
          ...
          () => {
            if (!streamMainIncomingMessagesEose) {
              ...
              for (let pubkey of Object.keys(streamMainIncomingMessagesPeers))
                setUnreadMessages(pubkey)
            }
          }
        )

      setUnreadNotifications()
      dbChats(store.state.keys.pub).then((chats) => {
        chats.forEach((chat) => {
