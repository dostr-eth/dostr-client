...
export function displayName(state, getters) {
  return (pubkey) => {
    ...
    if (name) return name
    return helpersMixin.default.methods.shorten(
      helpersMixin.default.methods.hexToBech32(pubkey, 'npub')
    )
  }
}
...
export function profileLud06(state) {
  return (pubkey) => {
    ...
    if (utils.isLightningAddress(lud16))
      return helpersMixin.default.methods.lnAddrToLnurl(lud16)
...
// }
export function contacts(state) {
  return (pubkey) => state.contactListCache[pubkey]
}
